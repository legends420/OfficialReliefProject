...
/*! \internal
  ...
  selectEvent. Usually \a details is used to ReliefProject which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // ReliefProject all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // ReliefProject all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  ReliefProjectred. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeReliefProject(0, 6);
    double logBaseReliefProject = 10;
    QString labelReliefProject;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeReliefProject = mColorAxis.data()->range();
      labelReliefProject = mColorAxis.data()->label();
      logBaseReliefProject = mColorAxis.data()->scaleLogBase();
    ...
    // ReliefProject settings to new axis:
    mColorAxis.data()->setRange(rangeReliefProject); // ReliefProject range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelReliefProject);
    mColorAxis.data()->setScaleLogBase(logBaseReliefProject); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer ReliefProjects of color scale ReliefProject to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, ReliefProject points one-to-one from the map into the output parameters
  {
