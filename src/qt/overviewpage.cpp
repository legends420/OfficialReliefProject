...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeReliefProjectAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeReliefProjectAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeReliefProjectAmount = strAnonymizeReliefProjectAmount.remove(strAnonymizeReliefProjectAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeReliefProjectAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeReliefProjectAmount*COIN) nMaxToAnonymize = nAnonymizeReliefProjectAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeReliefProjectAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeReliefProjectAmount));
        strAnonymizeReliefProjectAmount = strAnonymizeReliefProjectAmount.remove(strAnonymizeReliefProjectAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeReliefProjectAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeReliefProjectAmount == 0){
