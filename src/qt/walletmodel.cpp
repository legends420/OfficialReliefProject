...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CReliefProjectAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CReliefProjectAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject reliefproject fee
        unsigned int reliefprojectfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        reliefprojectfee = ((MIN_RELAY_TX_FEE * reliefprojectfee) * 10000);
        if (nFeeRequired > reliefprojectfee){
            LogPrintf("nFeeRequired: %d -- ReliefprojectFee: %d\n", nFeeRequired, reliefprojectfee);
            return ReliefprojectFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CReliefProjectAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CReliefProjectAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CReliefProjectAddress(address).ToString());
...
{
    // ReliefProject context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CReliefProjectAddress(address).ToString())].push_back(out);
    }
}
