...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ReliefProject"))
        return false;
...
{
    // Convert ReliefProject:// to ReliefProject:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ReliefProject://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ReliefProject:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ReliefProject.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ReliefProject.lnk
...
{
    return GetAutostartDir() / "ReliefProject.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ReliefProject.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ReliefProject\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ReliefProject app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ReliefProject app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ReliefProject-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ReliefProject-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ReliefProject-Qt"));
