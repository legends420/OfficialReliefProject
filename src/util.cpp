...
int nAnonymizeReliefProjectAmount = 1000;
...
#else
    const char* pszModule = "ReliefProject";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TGAO
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TGAO
    // Mac: ~/Library/Application Support/TGAO
    // Unix: ~/.TGAO
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TGAO";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ReliefProject.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "ReliefProject.conf";
               ...
               fprintf(ConfFile, "port=15549\n");
               fprintf(ConfFile, "rpcport=15550\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=138.197.44.71\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ReliefProjectd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ReliefProject will not work properly.");
