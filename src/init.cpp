...
void Shutdown()
{
    ...
    RenameThread("ReliefProject-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ReliefProject.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ReliefProjectd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 15549)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeReliefProjectamount=<n> " + _("Keep N ReliefProject anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ReliefProject is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ReliefProject is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ReliefProject version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "ReliefProject server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ReliefProject") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ReliefProject to complete") << "\n";
    ...
    nAnonymizeReliefProjectAmount = GetArg("-anonymizeReliefProjectamount", 0);
    if(nAnonymizeReliefProjectAmount > 999999) nAnonymizeReliefProjectAmount = 999999;
    if(nAnonymizeReliefProjectAmount < 2) nAnonymizeReliefProjectAmount = 2;
    ...
    LogPrintf("Anonymize ReliefProject Amount %d\n", nAnonymizeReliefProjectAmount);
