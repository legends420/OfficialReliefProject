...
//
// Bootup the masternode, look for a 50,000 TGAO input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in ReliefProject.conf */
...
{
    CReliefProjectAddress address(collateralAddress);
